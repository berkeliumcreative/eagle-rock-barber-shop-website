...
import { Scissors, Sparkles, Star, Flame, MapPin, Phone } from "lucide-react";

export default function Page() {
  return (
    <main className="min-h-screen bg-background text-foreground">
      <MinimalNav
        logo={<span><span style={{ color: "var(--theme-accent)" }}>Eagle Rock</span> Barber Shop</span>}
        items={[
      ...
      <StatsSection stats={[{ value: 93, suffix: "+", label: "5-Star Reviews" }, { value: 4.6, label: "Google Rating" }, { value: 7, label: "Days a Week" }]} />
      ...
      <section id="services" className="py-20 md:py-28 bg-card">
        <div className="max-w-6xl mx-auto px-6">
          <SectionHeading title="Our Services" subtitle="Classic and modern barbering" />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mt-12">
            {content.services.map((item: any, i: number) => {
              const icons = [<Scissors key="sc" className="w-8 h-8" />, <Sparkles key="sp" className="w-8 h-8" />, <Star key="st" className="w-8 h-8" />, <Flame key="f" className="w-8 h-8" />];
              return (
      ...
      <section id="reviews" className="py-20 md:py-28">
        <SectionHeading title="Eagle Rock Approved" subtitle="93+ reviews from the neighborhood" />
        <div className="mt-12">
          <TestimonialMarquee testimonials={content.reviews.map((r: any) => ({ quote: r.text, author: r.name, rating: r.rating }))} />
        </div>
      </section>

      <CTABanner heading="Walk In, Walk Out Sharp" subheading="No appointment needed. Open 7 days a week on Eagle Rock Blvd." ctaText="Call Now" ctaHref="tel:3234287374" variant="filled" />
          ...
          <div>
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <a href="tel:3234287374" className="text-muted-foreground hover:underline">{content.contact.phone}</a>
              </div>
            </div>
